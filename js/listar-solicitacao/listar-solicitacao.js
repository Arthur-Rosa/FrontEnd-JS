...
if (input.value != "") {
    if (input.value.match(/^\d{2}([./-])\d{2}\1\d{4}$/)) {
        url = 'http://10.92.198.38:8080/api/solic/buscar/palavra/' + FormataStringData(input.value) + '/' + atributosUrl[1]
    } else {
        url = 'http://10.92.198.38:8080/api/solic/buscar/palavra/' + input.value + '/' + atributosUrl[1]
    }
}
...
fetch('http://10.92.198.38:8080/api/solic/autocomplete')
    .then((resp) => {
        resp.json().then((resposta) => {
